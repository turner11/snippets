...
description: 'Surgical code refactoring to improve maintainability without changing behavior. Covers extracting functions, renaming variables, breaking down god functions, improving type safety, eliminating code smells, resulting high cohesion low coupling code, and applying design patterns. use for gradual improvements.'
...
Improve code structure and readability without changing external behavior. Refactoring is gradual evolution, not revolution. Use this for improving existing code, not rewriting from scratch result code with resulting high cohesion and low coupling.

YOu must always take into considerations also the 
...
If `$PATH` is a string representing a path to folder / file in project to be refactored

If `$PATH` use project root
...
\+ const ONE\_DAY\_MS = 24 * 60 * 60 * 1000;

\+

\+ if (user.status === UserStatus.INACTIVE) { /* ... */ }

\+ const discount = total * DISCOUNT\_RATES.PREMIUM;

\+ setTimeout(callback, ONE\_DAY\_MS);
